...
func TestTCPLargeBuffers(t *testing.T) {
	...
	verifyLargeBuffer := func(packetType, direction uint8, connInfo BpfConnectionInfoT, expectedBuf string) {
		buf, ok := extractTCPLargeBuffer(pctx, packetType, direction, connInfo)
	...
	// Verify normal write
	verifyLargeBuffer(firstEvent.PacketType, firstEvent.Direction, firstEvent.ConnInfo, firstBuf)
	...
	verifyLargeBuffer(firstEvent.PacketType, firstEvent.Direction, firstEvent.ConnInfo, secondBuf)

	// Verify second read error
	_, ok := extractTCPLargeBuffer(pctx, firstEvent.PacketType, firstEvent.Direction, firstEvent.ConnInfo)
	...
	// Verify no buffer read happens for different packet_type
	_, ok = extractTCPLargeBuffer(pctx, 3, firstEvent.Direction, firstEvent.ConnInfo)
	...
	verifyLargeBuffer(firstEvent.PacketType, firstEvent.Direction, firstEvent.ConnInfo, firstBuf+appendBuf)
	...
	require.NoError(t, err)
	verifyLargeBuffer(firstEvent.PacketType, firstEvent.Direction, firstEvent.ConnInfo, firstBuf+"foobar")
}
